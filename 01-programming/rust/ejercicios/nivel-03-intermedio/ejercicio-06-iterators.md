...
fn main() {
    ...
    let pares: Vec<i32> = datos.iter().filter(|&&n| n % 2 == 0).copied().collect();
    println!("Pares: {:?}", pares);

    let cuadrados: Vec<i32> = datos.iter().map(|&n| n * n).collect();
    ...
    let planos: Vec<i32> = (1..=3).flat_map(|n| [n, n * 10]).collect();
    println!("Flat map: {:?}", planos);

    let mayores = datos.iter().filter(|&&n| n > 3).count();
    println!("Mayores que 3: {}", mayores);
}
````
