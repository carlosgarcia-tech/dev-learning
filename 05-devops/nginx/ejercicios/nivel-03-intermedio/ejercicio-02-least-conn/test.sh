...
grep -Eq 'least_conn' "$CONF" || fail "falta directiva least_conn en upstream"
grep -Eq 'server[[:space:]]+127\.0\.0\.1:9001' "$CONF" || fail "falta server 127.0.0.1:9001"
grep -Eq 'server[[:space:]]+127\.0\.0\.1:9002' "$CONF" || fail "falta server 127.0.0.1:9002"
...
grep -Eq 'proxy_set_header[[:space:]]+X-Real-IP' "$CONF" || fail "falta proxy_set_header X-Real-IP"
...
if [ -n "$B1_PID" ] && [ -n "$B2_PID" ]; then
    ...
    echo "$R" | grep -q 'backend-' || fail "no se recibió respuesta de backend (vino: $R)"
fi
