#!/usr/bin/env bash
# Verificación: ejercicio-06-keys-y-dbsize
set -u
cd "$(dirname "$0")" || exit 1

CTR="redis-ej-$$"
SETUP="ejercicio-06-keys-y-dbsize-setup.redis"
SOL="ejercicio-06-keys-y-dbsize-solucion.redis"
EXPECTED="ejercicio-06-keys-y-dbsize-expected.txt"
...
podman exec -i "$CTR" redis-cli < "$SOL" 2>&1 | sort > tmp_actual.txt
