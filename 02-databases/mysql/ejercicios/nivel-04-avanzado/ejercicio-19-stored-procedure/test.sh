...
# Los stored procedures requieren MySQL (sintaxis DELIMITER, CREATE PROCEDURE).
# Si MySQL no está disponible, el test valida la consulta equivalente en SQLite.
set -euo pipefail
...
if [ "$USE_MYSQL" -eq 1 ]; then
  ...
  set +e
  # Extraer solo el primer bloque MYSQL-ONLY (el procedure real)
  awk '/-- MYSQL-ONLY START/{flag=1; next} /-- MYSQL-ONLY END/{flag=0} flag' solucion.sql > "$TMP/solucion_mysql.sql"
  mysql --batch --skip-column-names $MYSQL_ARGS "$DB_NAME" < "$TMP/solucion_mysql.sql" >"$TMP/actual" 2>"$TMP/err"
  rc=$?
  set -e
  if [ $rc -ne 0 ]; then
  ...
  # Extraer el segundo bloque MYSQL-ONLY (fallback SELECT)
