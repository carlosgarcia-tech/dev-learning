...
#   ejecuta setup + solución contra una base efímera (necesita replica set para transacciones).
...
if command -v mongosh >/dev/null 2>&1 && timeout 3 mongosh --quiet --eval "db.runCommand({ping:1}).ok" >/dev/null 2>&1; then
  ...
  if [[ "$IS_RS" == "1" ]]; then
    ...
    echo "AVISO: el servidor no es un replica set; las transacciones requieren replica set."
