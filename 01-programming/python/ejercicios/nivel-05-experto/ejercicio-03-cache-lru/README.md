...
Completa `main.py` para que implemente una caché LRU (Least Recently Used) con `collections.OrderedDict`:
...
4. Método `__len__` — devuelve el tamaño actual.
5. En `if __name__ == "__main__":`, demuestra el comportamiento: crea una caché de capacidad 2, pon `a=1`, `b=2`, accede a `a` (lo hace reciente), pon `c=3` (expulsa a `b`), y verifica que `get("b")` devuelve `None` y `get("a")` devuelve `1`.
...
- [ ] El elemento menos reciente se expulsa al superar la capacidad.
