#!/usr/bin/env bash
# test.sh — Ejercicio 03 nivel-03 (proxy_set_header y X-Forwarded-For).
# Nota: requiere nginx para validación completa; sin nginx valida sintaxis/estructura.
set -euo pipefail
cd "$(dirname "$0")" || exit 1

CONF="nginx.conf"
PORT=${TEST_PORT:-$((19400 + RANDOM % 1000))}
...
grep -Eq 'proxy_set_header[[:space:]]+Host[[:space:]]+\$host' "$CONF" || fail "falta proxy_set_header Host \$host"
grep -Eq 'proxy_set_header[[:space:]]+X-Real-IP[[:space:]]+\$remote_addr' "$CONF" || fail "falta proxy_set_header X-Real-IP \$remote_addr"
grep -Eq 'proxy_set_header[[:space:]]+X-Forwarded-For' "$CONF" || fail "falta proxy_set_header X-Forwarded-For"
grep -Eq 'proxy_set_header[[:space:]]+X-Forwarded-Proto' "$CONF" || fail "falta proxy_set_header X-Forwarded-Proto"

if ! command -v "$NGINX_BIN" >/dev/null 2>&1; then
    echo "OK Tests pasaron"; exit 0
fi

sed -e "s/127.0.0.1:9001/127.0.0.1:$B1_PORT/g" "$CONF" > "$RUN_CONF"
...
if [ -n "$B1_PID" ]; then
    B=$(curl -s --max-time 2 "http://127.0.0.1:$PORT/" || true)
    echo "$B" | grep -qi 'X-Forwarded-For' || fail "el backend no recibió X-Forwarded-For (vino: $B)"
fi
