...
1. Une `libros` con `autores` mediante `$lookup` (`localField: "autor_id"`, `foreignField: "_id"`), aplanad con `$unwind` y proyecta `_id: 0` con `titulo`, `anio`, `autor` y `nacionalidad`.
...
- [ ] Los tests pasan: `bash ejercicio-03-lookup-join-test.sh`
...
```bash
bash ejercicio-03-lookup-join-test.sh   # requiere podman (levanta mongo efímero)
```