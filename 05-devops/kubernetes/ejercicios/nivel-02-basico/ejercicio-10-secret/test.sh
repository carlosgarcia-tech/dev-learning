...
# Validaciones específicas del Secret
python3 - "$YAML_DIR" <<'PYEOF' || exit 1
import sys, yaml, glob, os, base64
yaml_dir = sys.argv[1]
files = sorted(glob.glob(os.path.join(yaml_dir, '*.yaml')))
docs = []
for f in files:
...
# Debe existir un Secret
if 'Secret' not in kinds:
    print("Falta un recurso Secret")
    sys.exit(1)
sec = kinds['Secret']

sec_name = sec.get('metadata', {}).get('name')
# type Opaque (o al menos definido)
stype = sec.get('type', 'Opaque')
if stype != 'Opaque':
    print(f"El Secret debe ser tipo Opaque (encontrado: {stype})")
    sys.exit(1)

data = sec.get('data', {})
if not data:
    print("El Secret debe tener claves en spec.data")
    sys.exit(1)
for key in ('API_KEY', 'DB_PASSWORD'):
    if key not in data:
        print(f"El Secret debe definir la clave '{key}'")
        sys.exit(1)
# Los valores deben ser base64 válido (decodificables)
for key, val in data.items():
    try:
        base64.b64decode(val, validate=True)
    except Exception:
        print(f"El valor de '{key}' en el Secret no es base64 válido")
        sys.exit(1)
...
# Debe inyectar variables de entorno desde el Secret
env = c.get('env', [])
ref_keys = set()
for e in env:
    ref = e.get('valueFrom', {}).get('secretKeyRef', {})
    if ref.get('name') == sec_name:
        ref_keys.add(ref.get('key'))
for key in ('API_KEY', 'DB_PASSWORD'):
    if key not in ref_keys:
        print(f"El Pod debe inyectar '{key}' desde secretKeyRef del Secret '{sec_name}'")
        sys.exit(1)
PYEOF

# Validación opcional con cluster K8s
if command -v kubectl >/dev/null 2>&1 && kubectl cluster-info >/dev/null 2>&1; then
  ns="ej10-$(date +%s | tail -c 6)"
  kubectl create namespace "$ns" >/dev/null 2>&1
  if kubectl apply -f "$YAML_DIR" -n "$ns" >/dev/null 2>&1; then
    kubectl get secret app-secret -n "$ns" >/dev/null 2>&1 || true
