#!/usr/bin/env bash
# Validación del ejercicio 04 - POST con validación (error 422).
set -euo pipefail
cd "$(dirname "$0")" || exit 1
...
errors = []
if r.get("status") != 422:
    errors.append(f"status debe ser 422, es {r.get('status')}")
...
for k in ("type", "title", "status"):
    if k not in b: errors.append(f"body falta '{k}'")
if b.get("status") != 422: errors.append("body.status debe ser 422")
errs = b.get("errors")
if not isinstance(errs, list) or len(errs) < 3:
    errors.append("body.errors debe ser un array con al menos 3 errores")
else:
    fields = set()
    for e in errs:
        for k in ("field", "code", "message"):
            if k not in e:
                errors.append(f"cada error debe tener '{k}': {e}")
        fields.add(e.get("field"))
    for f in ("name", "price", "stock"):
        if f not in fields:
            errors.append(f"falta error para el campo '{f}'")
if errors:
