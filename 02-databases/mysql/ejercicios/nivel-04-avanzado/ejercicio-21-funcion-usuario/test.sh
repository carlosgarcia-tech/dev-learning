...
# Las funciones definidas por usuario requieren MySQL. Si no está disponible,
# valida la consulta equivalente en SQLite.
set -euo pipefail
...
if [ "$USE_MYSQL" -eq 1 ]; then
  ...
  # Permitir creación de funciones
  mysql $MYSQL_ARGS -e "SET GLOBAL log_bin_trust_function_creators = 1;" 2>/dev/null || true
  set +e
