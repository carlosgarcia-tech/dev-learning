...
grep -Eq 'ip_hash' "$CONF" || fail "falta ip_hash en upstream"
grep -Eq 'server[[:space:]]+127\.0\.0\.1:9001' "$CONF" || fail "falta server 9001"
grep -Eq 'server[[:space:]]+127\.0\.0\.1:9002' "$CONF" || fail "falta server 9002"
