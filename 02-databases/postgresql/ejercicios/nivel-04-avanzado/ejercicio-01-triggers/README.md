...
1. Trigger para validar stock antes de un préstamo
2. Trigger para auditar cambios en préstamos
...
El trigger `tr_validar_stock_prestamo` original solo validaba pero nunca descontaba el stock, lo que dejaría burlar el control de inventario permitiendo N préstamos del último ejemplar disponible. Ahora descuenta `cantidad` dentro del propio `BEFORE INSERT`.
## Solución
...
```bash
cd ejercicios/nivel-04-avanzado/ejercicio-01-triggers
bash test.sh
```
