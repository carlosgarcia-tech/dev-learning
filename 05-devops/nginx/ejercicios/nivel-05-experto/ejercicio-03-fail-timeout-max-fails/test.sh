#!/usr/bin/env bash
# test.sh — Ejercicio 03 nivel-05 (fail_timeout y max_fails).
# Nota: requiere nginx para validación completa; sin nginx valida sintaxis/estructura.
set -euo pipefail
cd "$(dirname "$0")" || exit 1

CONF="nginx.conf"
PORT=${TEST_PORT:-$((21100 + RANDOM % 500))}
...
grep -Eq '^[[:space:]]*upstream[[:space:]]+' "$CONF" || fail "falta upstream"
grep -Eq 'max_fails=3' "$CONF" || fail "falta max_fails=3 en los servers"
grep -Eq 'fail_timeout=30s' "$CONF" || fail "falta fail_timeout=30s en los servers"
grep -Eq 'proxy_next_upstream' "$CONF" || fail "falta proxy_next_upstream"
grep -Eq 'http_502' "$CONF" || fail "proxy_next_upstream debe incluir http_502"
grep -Eq 'http_503' "$CONF" || fail "proxy_next_upstream debe incluir http_503"
grep -Eq 'http_504' "$CONF" || fail "proxy_next_upstream debe incluir http_504"
grep -Eq 'proxy_next_upstream_tries[[:space:]]+2' "$CONF" || fail "falta proxy_next_upstream_tries 2"
...
if [ -n "$B1_PID" ] && [ -n "$B2_PID" ]; then
    ...
    echo "$R" | grep -q 'backend-' || fail "no se recibió respuesta (vino: $R)"
fi
