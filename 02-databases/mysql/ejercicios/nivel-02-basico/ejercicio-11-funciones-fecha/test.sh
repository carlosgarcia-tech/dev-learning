...
# Este ejercicio usa funciones de fecha específicas de MySQL (YEAR, MONTH).
set -euo pipefail
...
if [ "$USE_MYSQL" -eq 1 ]; then
  ...
  adapt() {
    sed -E \
      ...
      -e 's/\)[[:space:]]*ENGINE=.*$/);/' \
      -e 's/\)[[:space:]]*DEFAULT .*$/);/' \
      -e 's/CAST\(YEAR\(([^)]*)\) AS CHAR\)/CAST(strftime("%Y", \1) AS TEXT)/g' \
      -e 's/CAST\(MONTH\(([^)]*)\) AS CHAR\)/CAST(CAST(strftime("%m", \1) AS INT) AS TEXT)/g' \
      -e "s/ COMMENT '[^']*'//g" \
      "$1"
  }
