...
# Los eventos requieren MySQL. Si no está disponible, valida la consulta de datos en SQLite.
# Nota: el test no ejecuta el evento (no espera 1 hora), solo verifica que se crea sin error
# y que la consulta de datos devuelve los resultados correctos.
set -euo pipefail
...
if [ "$USE_MYSQL" -eq 1 ]; then
  ...
  # Ejecutar; la salida incluye event_name de information_schema y luego los logs.
  # Comparamos solo las últimas 4 filas (los logs).
  mysql --batch --skip-column-names $MYSQL_ARGS "$DB_NAME" < "$TMP/solucion_mysql.sql" >"$TMP/actual_raw" 2>"$TMP/err"
  rc=$?
  set -e
  if [ $rc -ne 0 ]; then
  ...
  # Extraer solo las filas de datos (las que contienen fecha en formato YYYY-MM-DD)
  grep -E '^[0-9]+\|' "$TMP/actual_raw" | tail -4 > "$TMP/actual"
  mysql $MYSQL_ARGS -e "DROP DATABASE IF EXISTS $DB_NAME;" 2>/dev/null || true
else
