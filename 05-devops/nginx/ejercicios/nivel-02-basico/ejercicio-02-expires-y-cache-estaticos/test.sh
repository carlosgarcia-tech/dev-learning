#!/usr/bin/env bash
# test.sh — Ejercicio 02 nivel-02 (expires y cache): Cache-Control por tipo.
...
CONF="nginx.conf"
WEB_DIR="$PWD/web"
PORT=${TEST_PORT:-$((18700 + RANDOM % 1000))}
...
grep -Eq 'no-cache' "$CONF" || fail "falta Cache-Control no-cache para html"
grep -Eq 'immutable' "$CONF" || fail "falta Cache-Control immutable para css/js"
...
CC_HTML=$(cc_of "/index.html")

echo "$CC_CSS" | grep -qi 'max-age=31536000' || fail "/app.css Cache-Control debe contener max-age=31536000 (vino '$CC_CSS')"
echo "$CC_HTML" | grep -qi 'no-cache' || fail "/index.html Cache-Control debe contener no-cache (vino '$CC_HTML')"
