...
# EXPLAIN ANALYZE requiere MySQL 8.0+. Si no está disponible, usa EXPLAIN QUERY PLAN de SQLite.
# El test ejecuta EXPLAIN por separado (descarta salida) y compara solo la consulta de datos.
set -euo pipefail
...
if [ "$USE_MYSQL" -eq 1 ]; then
  ...
  # Extraer primer bloque MYSQL-ONLY
  awk '/-- MYSQL-ONLY START/{flag=1; next} /-- MYSQL-ONLY END/{flag=0} flag' solucion.sql > "$TMP/solucion_mysql.sql"
  # Ejecutar EXPLAIN ANALYZE por separado (descartar salida)
  EXPLAIN_STMT=$(grep -i '^EXPLAIN ' "$TMP/solucion_mysql.sql" | head -1 | sed 's/;[[:space:]]*$//')
  set +e
  mysql $MYSQL_ARGS "$DB_NAME" -e "$EXPLAIN_STMT" >/dev/null 2>"$TMP/err"
  rc=$?
  set -e
  if [ $rc -ne 0 ]; then
    echo "FAIL Tests fallaron (error ejecutando EXPLAIN ANALYZE)"
    cat "$TMP/err" >&2
    mysql $MYSQL_ARGS -e "DROP DATABASE IF EXISTS $DB_NAME;" 2>/dev/null || true
    exit 1
  fi
  # Ejecutar el SELECT de datos
  grep -v -i '^EXPLAIN ' "$TMP/solucion_mysql.sql" > "$TMP/data_sql.sql"
  set +e
  mysql --batch --skip-column-names $MYSQL_ARGS "$DB_NAME" < "$TMP/data_sql.sql" >"$TMP/actual" 2>"$TMP/err"
  rc=$?
  set -e
  if [ $rc -ne 0 ]; then
  ...
  # Segundo bloque MYSQL-ONLY (fallback SQLite)
  awk '/-- MYSQL-ONLY START/{c++} c==2 && !/-- MYSQL-ONLY (START|END)/{print} /-- MYSQL-ONLY END/ && c==2{c++}' solucion.sql > "$TMP/solucion_lite.sql"
  adapt "$TMP/solucion_lite.sql" > "$TMP/solucion_adaptada.sql"
  set +e
  {
    printf '.nullvalue NULL\n.mode list\n.headers off\n.separator "\t"\nPRAGMA foreign_keys=ON;\n'
    cat "$TMP/schema_lite.sql"
    cat "$TMP/solucion_adaptada.sql"
  } | sqlite3 "$DBFILE" 2>"$TMP/err" | grep -v -E '^(SCAN|SEARCH|USE|COVERING|LIST)' > "$TMP/actual"
  rc=${PIPESTATUS[0]}
  set -e
  if [ $rc -ne 0 ]; then
