...
# Validaciones específicas del Service ClusterIP
python3 - "$YAML_DIR" <<'PYEOF' || exit 1
import sys, yaml, glob, os
yaml_dir = sys.argv[1]
files = sorted(glob.glob(os.path.join(yaml_dir, '*.yaml')))
...
# El Deployment debe tener al menos 2 réplicas
...
# El Service debe ser tipo ClusterIP
svc_type = svc.get('spec', {}).get('type', 'ClusterIP')
if svc_type != 'ClusterIP':
    print(f"El Service debe ser tipo ClusterIP (encontrado: {svc_type})")
    sys.exit(1)

# El Service debe tener selector
selector = svc.get('spec', {}).get('selector')
if not selector:
    print("El Service debe definir spec.selector")
    sys.exit(1)

# El selector del Service debe coincidir con las labels de la plantilla del pod
...
# El Service debe exponer al menos un puerto con targetPort 80
ports = svc.get('spec', {}).get('ports', [])
if not ports:
    print("El Service debe definir al menos un puerto en spec.ports")
    sys.exit(1)
port_ok = any(p.get('port') == 80 and p.get('targetPort') == 80 for p in ports)
if not port_ok:
