...
# El particionamiento requiere MySQL. Si no está disponible, valida la tabla equivalente en SQLite.
set -euo pipefail
...
if [ "$USE_MYSQL" -eq 1 ]; then
  ...
  adapt() {
    sed -E \
      ...
      -e 's/INT NOT NULL AUTOINCREMENT/INTEGER/g' \
      -e 's/INTEGER,/,/g' \
      -e 's/\)[[:space:]]*ENGINE=.*$/);/' \
      -e 's/\)[[:space:]]*DEFAULT .*$/);/' \
  ...
  # El schema.sql está vacío. Usar el segundo bloque MYSQL-ONLY de solucion.sql.
