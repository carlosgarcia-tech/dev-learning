#!/usr/bin/env bash
# Verificación: ejercicio-02-where-y-orden
set -u
cd "$(dirname "$0")" || exit 1

DB="tmp.db"
SCHEMA="ejercicio-02-where-y-orden-schema.sql"
SOL="ejercicio-02-where-y-orden-solucion.sql"
EXPECTED="ejercicio-02-where-y-orden-expected.txt"
