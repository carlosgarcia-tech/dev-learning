...
# EXPLAIN produce salida variable: el test compara solo la consulta final
# (ignora las filas que produce EXPLAIN, que en --batch van al principio).
# Estrategia: ejecutar EXPLAIN por separado (se descarta su salida) y luego
# la consulta de datos, comparando solo esta última.
set -euo pipefail
...
# Separar solucion.sql: primera sentencia (EXPLAIN) y el resto (SELECT de datos).
# El test ejecuta EXPLAIN por separado (descartando su salida) y luego el SELECT.
split_solucion() {
  awk '
    /^EXPLAIN / { in_explain=1 }
    in_explain && /;[[:space:]]*$/ { print; in_explain=0; next }
    { print > (in_explain ? "/dev/null" : "'"$TMP/data_sql"'") }
  ' "$1"
}
...
if [ "$USE_MYSQL" -eq 1 ]; then
  ...
  # Extraer la primera sentencia EXPLAIN y el resto
  : > "$TMP/data_sql"
  awk '
    BEGIN { in_explain=0 }
    /^EXPLAIN / { in_explain=1 }
    in_explain && /;[[:space:]]*$/ { in_explain=0; next }
    { print > "'"$TMP/data_sql"'" }
  ' solucion.sql

  # Ejecutar EXPLAIN por separado (descartar salida)
  EXPLAIN_STMT=$(grep -i '^EXPLAIN ' solucion.sql | head -1 | sed 's/;[[:space:]]*$//')
  set +e
  mysql --batch --skip-column-names $MYSQL_ARGS "$DB_NAME" -e "$EXPLAIN_STMT" >/dev/null 2>"$TMP/err"
  rc=$?
  set -e
  if [ $rc -ne 0 ]; then
    echo "FAIL Tests fallaron (error ejecutando EXPLAIN)"
    cat "$TMP/err" >&2
    mysql $MYSQL_ARGS -e "DROP DATABASE IF EXISTS $DB_NAME;" 2>/dev/null || true
    exit 1
  fi

  # Ejecutar el resto (SELECT de datos)
  set +e
  mysql --batch --skip-column-names $MYSQL_ARGS "$DB_NAME" < "$TMP/data_sql" >"$TMP/actual" 2>"$TMP/err"
  rc=$?
  set -e
  if [ $rc -ne 0 ]; then
  ...
  adapt() {
    sed -E \
      ...
      -e 's/,\s*INDEX [a-zA-Z0-9_]+ \([a-zA-Z0-9_]+\)//g' \
      -e 's/INDEX [a-zA-Z0-9_]+ \([a-zA-Z0-9_]+\)//g' \
      -e 's/\)[[:space:]]*ENGINE=.*$/);/' \
      -e 's/\)[[:space:]]*DEFAULT .*$/);/' \
  ...
  awk '
    ...
    { print > "'"$TMP/data_sql"'" }
  ' solucion.sql
  adapt "$TMP/data_sql" > "$TMP/solucion_lite.sql"

  set +e
  {
    printf '.nullvalue NULL\n.mode list\n.headers off\n.separator "\t"\nPRAGMA foreign_keys=ON;\n'
    cat "$TMP/schema_lite.sql"
    # EXPLAIN (descartar salida)
    EXPLAIN_STMT=$(grep -i '^EXPLAIN ' solucion.sql | head -1 | sed 's/;[[:space:]]*$//')
    printf '%s;\n' "$EXPLAIN_STMT" | sed 's/EXPLAIN /EXPLAIN QUERY PLAN /I'
    # SELECT de datos
    cat "$TMP/solucion_lite.sql"
  } | sqlite3 "$DBFILE" 2>"$TMP/err" | grep -v -E '^(SCAN|SEARCH|USE|COVERING)' > "$TMP/actual"
  rc=${PIPESTATUS[0]}
  set -e
  if [ $rc -ne 0 ]; then
