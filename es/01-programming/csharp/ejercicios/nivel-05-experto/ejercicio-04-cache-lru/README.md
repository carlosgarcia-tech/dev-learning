...
Completa `Program.cs`. Implementa una **caché LRU** (Least Recently Used) con capacidad fija. La estructura se basa en un `Dictionary<string,string>` (valores) y un `LinkedList<string>` (orden de uso: la **primera** es la menos usada y la **última** la más reciente).
