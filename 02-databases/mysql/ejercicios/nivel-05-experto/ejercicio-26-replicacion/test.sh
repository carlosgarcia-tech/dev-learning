...
# La replicación requiere dos instancias de MySQL. Este ejercicio es teórico-práctico:
# el test valida que la consulta de verificación se ejecute correctamente.
set -euo pipefail
...
# Extraer solo las líneas no comentadas de solucion.sql (la consulta ejecutable)
grep -v '^[[:space:]]*--' solucion.sql | grep -v '^[[:space:]]*$' > "$TMP/executable.sql"
...
if [ "$USE_MYSQL" -eq 1 ]; then
  ...
  set +e
  mysql --batch --skip-column-names $MYSQL_ARGS "$DB_NAME" < "$TMP/executable.sql" >"$TMP/actual" 2>"$TMP/err"
  rc=$?
  set -e
  if [ $rc -ne 0 ]; then
