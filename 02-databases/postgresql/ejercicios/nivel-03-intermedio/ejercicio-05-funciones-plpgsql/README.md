# Ejercicio 05 — Funciones PL/pgSQL
...
1. Función para calcular la multa de un préstamo
2. Función para obtener estadísticas de un libro
...
La función `obtener_estadisticas_libro` original calculaba el promedio con `p.fecha_devolucion - p.fecha_prestamo`, lo cual da `NULL` para préstamos aún activos (fecha_devolucion IS NULL) y descuadra el promedio; se usa `COALESCE(p.fecha_devolucion, CURRENT_DATE)` para tratarlos como 'todavía en curso hasta hoy'.
## Solución
...
```bash
cd ejercicios/nivel-03-intermedio/ejercicio-05-funciones-plpgsql
bash test.sh
```
