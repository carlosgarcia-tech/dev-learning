...
# Simula un ciclo backup/restore creando una tabla de respaldo y copiando datos.
# MySQL: usa CREATE TABLE ... LIKE. SQLite: usa CREATE TABLE ... AS SELECT.
set -euo pipefail
...
if [ "$USE_MYSQL" -eq 1 ]; then
  ...
  adapt() {
    sed -E \
      ...
      -e 's/CREATE TABLE clientes_backup LIKE clientes;/CREATE TABLE clientes_backup AS SELECT * FROM clientes;/g' \
      -e 's/\)[[:space:]]*ENGINE=.*$/);/' \
      -e 's/\)[[:space:]]*DEFAULT .*$/);/' \
